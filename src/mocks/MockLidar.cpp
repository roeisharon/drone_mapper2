...
namespace {
    ...
    // Returns the number of beams on a given circle in the LIDAR's field of view.
...
// Returns the horizontal angle offset for a beam at a given offset and distance.
...
// Returns the altitude angle offset for a beam at a given offset and distance.
...
types::LidarConfigData MockLidar::config() const {
    ...
    // Returns a vector of LidarHit results for a scan at the given orientation.
types::LidarScanResult MockLidar::scan(Orientation scan_orientation) const {
    ...
    // Generate beams in concentric circles around the center beam.
    for (std::size_t circle = 1; circle < config_.fov_circles; ++circle) {
        ...
        // Compute the angle offsets for each beam in the circle and trace them.
...
// Traces a single LIDAR beam from the drone's position in the given orientation, returning the distance to the first occupied voxel or max distance if none is found.
